...
function print_results(m, frac, nodeLabelSize, edgeLabelSize, T)
    ...
    # buildSankeyGraph(returnedRecipes, nodeLabelSize, edgeLabelSize, T, objective_value(m))
...
function buildSankeyGraph(returnedRecipes, nodeLabelSize, edgeLabelSize, T, objValue)
        ...
        for (p1, qty1) in r1.out, (p2, qty2) in r2.in
            if p1 == p2
                ...
                if Graphs.is_cyclic(g)
                    ...
                    push!(sources, indices[r1])
                    push!(dests, indices[r2])
                    push!(weights, min(qty1 * used1, qty2 * used2))
                end
            end
        end
    end
    ...
    node_colors = palette(:seaborn_colorblind, length(labels))[:]
    ...
    sankey(sources, dests, weights; node_labels=labels, label_position=:bottom, compact=false, label_size=4, node_colors, edge_color=:gradient)
end