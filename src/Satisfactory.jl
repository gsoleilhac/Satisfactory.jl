...
function maximize!(::Type{T} ; 
    ...
    @objective(m, Max, x[T] - sum(qty * y[r] for (r, qty) in dependantRecipes(T)) - 1e-6 * sum(r.duration * y[r] for r in _recipes))
    # @objective(m, Max, x[T])
...
function maximizeDiscrete!(::Type{T}, frac = 1/12 ; 
    resources = Dict(), availableProducts = Dict(),
    lockedRecipes = String[], allowMultiRecipes = true) where T <: Product
    
    m = Model(CPLEX.Optimizer)
    @variable(m, 0 <= x[p in subtypes(Product)] <= 10000)
    @variable(m, 0 <= y[r in _recipes] <= 10000, Int)
